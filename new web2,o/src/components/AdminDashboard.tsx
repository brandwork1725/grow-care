...
export const AdminDashboard: React.FC = () => {
  ...
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState(() => {
    return sessionStorage.getItem("gc_admin_authenticated") === "true";
  });
  const [adminEmail, setAdminEmail] = useState("owner@growcare.com");
  const [adminPasscode, setAdminPasscode] = useState("");
  const [authError, setAuthError] = useState("");

  const handleAdminLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (adminPasscode === "admin123" || adminPasscode === "admin" || adminPasscode === "GROWCARE2026") {
      setIsAdminLoggedIn(true);
      sessionStorage.setItem("gc_admin_authenticated", "true");
      setAuthError("");
    } else {
      setAuthError("Invalid master security passcode. Enter 'admin123' or click One-Click Demo Auth.");
    }
  };

  const handleAdminLogout = () => {
    setIsAdminLoggedIn(false);
    sessionStorage.removeItem("gc_admin_authenticated");
    setAdminPasscode("");
  };
  ...
  if (!isAdminLoggedIn) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md overflow-y-auto animate-fadeIn">
        <div className="relative w-full max-w-md bg-[#FAF8F5] dark:bg-neutral-900 border border-black/10 dark:border-white/10 shadow-2xl p-8 space-y-6 font-sans">
          
          {/* Gate Header */}
          <div className="flex items-center justify-between border-b border-black/10 dark:border-white/10 pb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 border border-[#D4AF37]/50 bg-[#0A0A0A] flex items-center justify-center text-[#D4AF37]">
                <Lock className="w-5 h-5" />
              </div>
              <div>
                <h2 className="font-serif font-medium text-base text-[#0A0A0A] dark:text-[#F5F2ED]">
                  Restricted Staff Portal
                </h2>
                <div className="text-[10px] text-[#D4AF37] uppercase tracking-[0.2em] font-semibold">
          ...
          <p className="text-xs text-neutral-600 dark:text-neutral-400 font-light leading-relaxed">
            This admin suite is isolated from general store visitors. Please enter your master security credentials to manage store inventory, customer orders, and promotions.
          </p>

          {authError && (
            <div className="p-3 bg-rose-500/10 border border-rose-500/30 text-rose-600 dark:text-rose-400 text-xs font-semibold flex items-center gap-2">
          ...
          <form onSubmit={handleAdminLogin} className="space-y-4 text-xs">
            <div>
              <label className="font-semibold uppercase tracking-wider text-[10px] text-neutral-600 dark:text-neutral-400 block mb-1">
              ...
              <input
                ...
                placeholder="owner@growcare.com"
                className="w-full px-3.5 py-2.5 bg-white dark:bg-black border border-black/20 dark:border-white/20 text-[#0A0A0A] dark:text-white font-sans focus:outline-none focus:border-[#D4AF37]"
              />
            </div>
            ...
            <div className="pt-2 space-y-2">
              <button
                type="submit"
                className="editorial-btn-primary w-full py-3 text-xs uppercase tracking-[0.15em] cursor-pointer flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="editorial-btn-secondary w-full py-2.5 text-xs uppercase tracking-wider cursor-pointer"
              >
                One-Click Quick Admin Access (Demo)
              </button>
            </div>
          </form>

          <div className="pt-2 border-t border-black/10 dark:border-white/10 text-center">
            <button
              onClick={() => setIsAdminOpen(false)}
              className="text-xs text-neutral-500 hover:text-[#0A0A0A] dark:hover:text-white transition cursor-pointer"
            >
        ...
        <div className="px-6 py-5 border-b border-black/10 dark:border-white/10 bg-[#0A0A0A] text-[#F5F2ED] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 border border-white/20 hover:border-rose-500 text-neutral-300 hover:text-rose-400 text-xs font-semibold uppercase tracking-wider transition cursor-pointer"
